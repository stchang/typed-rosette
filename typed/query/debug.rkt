#lang turnstile
(require
 (prefix-in t/ro: (only-in "../rosette.rkt"
...
(define-typed-syntax define/debug #:datum-literals (: -> →)
  [(_ x:id e) ≫
   ...
   --------
   [≻ (begin-
        (define-syntax- x (make-rename-transformer (assign-type #'y #'τ #:wrap? #f)))
  ...
  [(_ (f [x : ty] ...) (~or → ->) ty_out e ...+) ≫
   ...
   --------
   [≻ (begin-
        (define-syntax- f (make-rename-transformer (assign-type #'f- #'(t/ro:C→ ty ... ty_out) #:wrap? #f)))
        (ro:define/debug f-
          (t/ro:λ ([x : ty] ...)
